...
| Upload size limit | `nginx/nginx.conf` — `client_max_body_size 50m` | Applied on both `/api/` and `/graphql` locations. FastAPI itself has no built-in size cap; Nginx is the enforcement point. |
...
| ECS task role SQS scope | `terraform/modules/iam/main.tf` | FastAPI app: `sqs:SendMessage` + `sqs:GetQueueAttributes` only. Lambda worker: `ReceiveMessage` + `DeleteMessage`. Never cross-assign. |
...
**`migrate.sh`** — interactive migration helper for local development (runs outside Docker, requires Alembic in the active virtual environment). Prompts for a migration message, runs `alembic revision --autogenerate`, then asks whether to immediately apply with `alembic upgrade head`.
...
# To stop: docker stop fastapi-boilerplate-<service>
```
...
The `app` service in `docker-compose.yml` uses `Dockerfile.dev`, which starts FastAPI under `debugpy` on port 5678. The debugger is always available while the stack is running.

```bash
docker compose up --build
```

Then launch **"Docker: Attach to FastAPI"** in VSCode. Source changes are picked up immediately via the volume mount (no rebuild needed).

### Option 2 — Run FastAPI on the host

Start only infrastructure, then launch **"Local: FastAPI Debug"** in VSCode (F5). The `preLaunchTask` runs `docker compose up -d postgres localstack migrate` automatically; `postDebugTask` stops them when the session ends.
...
| `app` | 5000, 5678 | FastAPI app (uvicorn + debugpy on 5678) — also reachable directly on 5000 |
| `postgres` | 5432 | Primary database |
| `migrate` | — | One-shot container: runs `alembic upgrade head` on startup |
...
Other services reach the FastAPI app internally via `http://app:5000` (direct) or `http://nginx/api/v1/` (through the proxy). External clients always hit port 80.
...
Build context stays at `.` (repo root) so Dockerfiles can access `requirements.txt`, `migrations/`, `alembic.ini`, and `wsgi.py`. Only the `dockerfile:` path points into `services/`:
...
`app` and `migrate` services mount `./services/app:/app`. The `migrate` service also mounts `./migrations:/app/migrations` and `./alembic.ini:/app/alembic.ini` since Alembic files stay at the repo root.
...
- **REST:** `services/app/app/api/v1/` — `APIRouter` mounted at `/api/v1/`
- **GraphQL:** `services/app/app/graphql_api/` — Strawberry `GraphQLRouter` at `/graphql`, with `multipart_uploads_enabled=True` for file uploads

GraphQL resolvers do not use `Depends(require_access_token)` (a FastAPI dependency) — they call `get_token_from_bearer()` / `verify_access_token()` from `services/app/app/security.py` manually at the top of each resolver, since Strawberry handles the request context differently. The JWT identity and request are available via `info.context.request.headers` and `info.context.db`.
...
All models must be imported in `services/app/app/models/__init__.py` for Alembic to detect them via `Base.metadata`. Current models:
...
**Loki labels:** every event is tagged with `{app: "fastapi-boilerplate", env: <config_name>, level: <info|warning|error>}`. Query in Grafana Explore with `{app="fastapi-boilerplate"}` or `{level="error"}`.
...
**Sentry notes:**
- JWT auth failures return 401 from `services/app/app/security.py` — Sentry captures these via `FastApiIntegration` automatically.
...
```
FastAPI /metrics  ──scrape──►  Prometheus  ──PromQL──►  Grafana
...
- `grafana/dashboards/fastapi-app.json` — FastAPI App dashboard: request rate, error rate, p95/p99 latency stats, request rate by status/handler, latency by handler, error logs, all logs. Metrics use `prometheus-fastapi-instrumentator` names (`http_requests_total`, `http_request_duration_seconds_bucket`).
...
- Registers automatically via `Instrumentator().instrument(app).expose(app)` in `create_app()`.
- Exposes `/metrics` in Prometheus text format.
- Key metrics: `http_requests_total{method, status, handler}`, `http_request_duration_seconds_bucket{method, status, handler, le}`.
...
| **Extensions** | `services/app/app/extensions.py` | Defines SQLAlchemy `Base` (DeclarativeBase). Engine + session factory live in `services/app/app/database.py` and are initialized by `create_app()` |
...
| **Logging** | `services/app/app/logging/` | `AppLogger` + logger adapters (`ConsoleLogger`, `SentryLogger`, `CloudWatchLogger`, `LokiLogger`), `mask_sensitive` data filter. Logger attached to `app.state.logger_adapter`; accessed via `request.app.state.logger_adapter` in handlers |
...
└── app/
    ...
    ├── integration/   — FastAPI TestClient (starlette) + SQLite in-memory (no Docker needed)
...
```bash
# Install test dependencies (includes pytest, httpx2, pytest-cov, requests)
...
|---|---|---|
| `app` | session | `create_app("testing")` with `dependency_overrides[get_db]` pointing at SQLite in-memory + `StaticPool` |
...
| `client` | function | `starlette.testclient.TestClient` wrapping the FastAPI app |
| `db_session` | function | Direct SQLAlchemy session for seeding data or asserting DB state |
...
| `mock_cache` | function | Attaches a `MagicMock` as `app.state.cache`, restores `None` after the test |
...
**GraphQL vs REST auth** — resolvers call `get_token_from_bearer()` + `verify_access_token()` from `app/security.py` manually, not via `Depends(require_access_token)`. GraphQL always returns HTTP 200 — success/failure lives in `response.data.<resolver>.success`. Integration tests POST to `/graphql` with the `Authorization` header directly.

**SQLite + UUID columns** — all UUID columns use SQLAlchemy's generic `Uuid` type which works transparently with both PostgreSQL and SQLite in tests.
...
**Responses:**
- REST: always use `rest_api_response(success, message, data, status_code, request=request)` from `services/app/app/api/utils/utils.py`. Returns a `JSONResponse` — never return a raw dict.
- GraphQL: always return `Response[T]` with `success`, `message`, and optionally `data`. Never raise exceptions from resolvers.

**Authentication:**
- REST handlers: add `user_id: str = Depends(require_access_token)` (or `require_refresh_token`) from `services/app/app/security.py`.
- GraphQL resolvers: call `get_token_from_bearer(auth_header)` + `verify_access_token(token)` / `verify_refresh_token(token)` inside a `try/except` at the top of the resolver — FastAPI's `Depends()` cannot be used inside Strawberry resolvers.
...
2. Generate and apply a migration: `alembic revision --autogenerate -m "description" && alembic upgrade head`.
3. Add any external service logic to `services/app/app/services/`.
4. Add a FastAPI router in `services/app/app/api/v1/` and include it in `services/app/app/api/v1/__init__.py`.
...
├── main.tf           # Wires all modules; creates JWT + FastAPI app secrets in Secrets Manager
...
terraform init
terraform apply -var="bucket_name=myorg-fastapi-boilerplate-tfstate"
...
- `SECRET_KEY` (FastAPI) — 64-char random string
...
| `ecs-task` | FastAPI app | S3 read/write (media bucket), SQS send/receive, CloudWatch logs |
