...
def _upload_file(client, headers):
    """Post a multipart GraphQL upload; returns the response."""
    ...
    return client.post(
        ...
        files={"0": ("test.jpg", io.BytesIO(b"test content"), "image/jpeg")},
        headers=headers,
    )
...
class TestSignedUrl:
    ...
    def test_another_users_media_returns_not_found(self, client, gql):
        ...
        headers1 = {
            "Authorization": f"Bearer {login1.json()['data']['login']['data']['accessToken']}"
        }
        ...
        with (
            ...
            media_id = _upload_file(client, headers1).json()["data"]["uploadFile"][
                "data"
            ]["mediaId"]
        ...
        headers2 = {
            "Authorization": f"Bearer {login2.json()['data']['login']['data']['accessToken']}"
        }

        payload = gql(_SIGNED_URL, {"mediaId": media_id}, headers=headers2).json()[
            "data"
        ]["signedUrl"]
        assert payload["success"] is False
        assert "Not found" in payload["message"]
