...
def _upload(client, headers, content=b"hello", filename="test.jpg"):
...
class TestUpload:
    ...
    def test_no_file_returns_422(self, client, auth_headers):
        # FastAPI returns 422 when a required UploadFile field is missing
        res = client.post("/api/v1/media/upload", headers=auth_headers)
    ...
    def test_media_record_persisted_in_db(self, client, auth_headers, db_session):
        from app.models.media import Media

        res = _upload(client, auth_headers)
        media_id = res.json()["data"]["media_id"]

        record = db_session.get(Media, uuid.UUID(media_id))
...
class TestGetUrl:
    ...
    def test_another_users_media_returns_404(self, client):
        ...
        media_id = _upload(client, {"Authorization": f"Bearer {token1}"}).json()[
            "data"
        ]["media_id"]
