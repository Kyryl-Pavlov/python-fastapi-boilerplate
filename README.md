<div align="center">
  <img src="docs/favicon.svg" width="80" height="80" alt="Python FastAPI Boilerplate"/>
</div>

# python-fastapi-boilerplate

A production-ready **Python 3.12 / FastAPI** boilerplate for building scalable **REST** and **GraphQL** APIs — engineered for teams who want a clean architecture without spending weeks on infrastructure.
...
- REST API (FastAPI `APIRouter`, versioned at `/api/v1/`) with auto-generated OpenAPI docs at `/docs`
...
- Async event processing via **SQS + Lambda** — FastAPI publishes events to SQS; a Lambda function (locally: a worker container) consumes them and writes to Postgres
...
│       │   ├── extensions.py        # SQLAlchemy Base (DeclarativeBase)
...
│       │   ├── api/
│       │   │   └── v1/              # FastAPI routers (auth, media, events, cache, health)
│       │   └── graphql_api/
...
├── alembic.ini              # Alembic config (accessible to Docker build context)
...
> **Why `wsgi.py`, `migrations/`, `alembic.ini`, and `requirements*.txt` stay at the repo root:** Docker build context is anchored at `.` so all Dockerfiles can reach them via COPY. The `migrate` service also bind-mounts `./alembic.ini` and `./migrations` so Alembic can run correctly.
...
If you want to run FastAPI directly on your machine (debug Option 2), you also need:

| Tool | Version | Purpose | Download |
|---|---|---|---|
| **Python** | 3.12+ | Runs the FastAPI app and installs dependencies | [python.org](https://www.python.org/downloads/) |
...
> `AWS_SQS_ENDPOINT_URL` and `SQS_QUEUE_URL` are injected by Docker Compose automatically for the `app` and `worker` containers. Add them to `.env.local` only when running the FastAPI app directly on the host (debug Option 2), replacing `localstack` with `localhost`.
...
| FastAPI app (direct) | http://localhost:5000 | Bypass Nginx for debugging |
...
All external traffic enters through **Nginx on port 80** (`nginx/nginx.conf`). The FastAPI app is not exposed publicly — Nginx proxies to it internally at `app:5000`.
...
1. Create `services/<name>/` with Dockerfile.
...
```
FastAPI app  →  SQS queue  →  Lambda (locally: worker container)  →  PostgreSQL
```
...
**File uploads are restricted** — only `jpg`, `jpeg`, `png`, `gif`, `webp`, `pdf`, `mp4`, `mov` are accepted. Anything else returns `415`. Maximum upload size is 50 MB (`MAX_CONTENT_LENGTH` in config). To extend the allowlist, edit `ALLOWED_EXTENSIONS` in [app/api/v1/media.py](app/api/v1/media.py).
...
| Non-root containers | FastAPI app runs as `app` system user; Lambda runs as `nobody` — no root access if a container is compromised |
| ECS task role scoped to send-only SQS | FastAPI app can only `SendMessage`. Only the Lambda worker can `ReceiveMessage` and `DeleteMessage` |
...
└── app/
    ...
    ├── integration/   — FastAPI TestClient (starlette) + SQLite in-memory (< 2 s total)
...
**Adding a new microservice:** create `services/<name>/` with its Dockerfile, add a build step to `build` using `context: .` and `file: services/<name>/Dockerfile`, a `run_migration` call in both `migrate-*` jobs if it has its own DB, and a deploy step at the correct tier in both `deploy-*` jobs. Workers go in `deploy-workers-*`.
...
- All secrets (DB password, JWT key, FastAPI secret key) are stored in AWS Secrets Manager and injected into containers at startup — never in environment files or task definitions
...
#    docker build -t <registry>/fastapi-boilerplate-dev/app:latest .
#    docker push <registry>/fastapi-boilerplate-dev/app:latest
...
The `app` service runs FastAPI under `debugpy` (port 5678) and mounts the source directory as a volume, so code changes are reflected immediately without rebuilding.
...
3. Select **"Docker: Attach to FastAPI"** and press **F5**.
...
Run only the infrastructure in Docker and the FastAPI app directly on your machine. This can give a faster feedback loop.
...
2. Select **"Local: FastAPI Debug"** and press **F5**.

VSCode automatically:
- Starts Postgres, LocalStack, and runs migrations before launch (`preLaunchTask`)
- Launches FastAPI under `debugpy` on port 5000 with hot reload enabled
- Stops the infrastructure containers when you stop the debugger (`postDebugTask`)

**Note on hostnames:** when FastAPI runs on the host it cannot resolve Docker service names like `postgres` or `localstack`. The launch config overrides those env vars automatically:
...
The FastAPI app exposes a Prometheus-format metrics endpoint at `http://localhost:5000/metrics`. Prometheus scrapes it every 15 seconds. Query directly at `http://localhost:9090`.
...
> `prometheus-fastapi-instrumentator` disables itself when uvicorn's reloader is active. `DEBUG_METRICS=1` (already set in `docker-compose.yml`) re-enables it for local dev.
...
```logql
{app="fastapi-boilerplate"}                    # all logs
{app="fastapi-boilerplate", level="error"}     # errors only
{app="fastapi-boilerplate"} |= "upload"        # text search
```
...
The production Docker image uses `services/app/Dockerfile` (uvicorn, no debugpy). To smoke-test it in isolation:
